...
func (s *Subnet) GetRandomAddress(poolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool) (IP, IP, string, error) {
	return s.GetRandomAddressWithIPFamily(poolName, podName, nicName, mac, skippedAddrs, checkConflict, "")
}

func (s *Subnet) GetRandomAddressWithIPFamily(poolName, podName, nicName string, mac *string, skippedAddrs []string, checkConflict bool, ipFamily string) (IP, IP, string, error) {
	s.Mutex.Lock()
	defer func() {
