...
func (ipam *IPAM) GetRandomAddress(podName, nicName string, mac *string, subnetName, poolName string, skippedAddrs []string, checkConflict bool) (string, string, string, error) {
	return ipam.GetRandomAddressWithIPFamily(podName, nicName, mac, subnetName, poolName, skippedAddrs, checkConflict, "")
}

func (ipam *IPAM) GetRandomAddressWithIPFamily(podName, nicName string, mac *string, subnetName, poolName string, skippedAddrs []string, checkConflict bool, ipFamily string) (string, string, string, error) {
	ipam.mutex.Lock()
	...
	v4IP, v6IP, macStr, err := subnet.GetRandomAddressWithIPFamily(poolName, podName, nicName, mac, skippedAddrs, checkConflict, ipFamily)
	if v4IP != nil {
		v4 = v4IP.String()
	}
	if v6IP != nil {
...
func (ipam *IPAM) GetStaticAddress(podName, nicName, ip string, mac *string, subnetName string, checkConflict bool) (string, string, string, error) {
	return ipam.GetStaticAddressWithIPFamily(podName, nicName, ip, mac, subnetName, checkConflict, "")
}

func (ipam *IPAM) GetStaticAddressWithIPFamily(podName, nicName, ip string, mac *string, subnetName string, checkConflict bool, ipFamily string) (string, string, string, error) {
	ipam.mutex.Lock()
	defer ipam.mutex.Unlock()
	var subnet *Subnet
	var ok bool
	...
	parsedIPs := make([]IP, 0, len(ipStrs))
	for _, ipStr := range ipStrs {
		ipAddr, err := NewIP(ipStr)
		if err != nil {
		...
		parsedIPs = append(parsedIPs, ipAddr)
	}
	...
	completedIPs, err := checkAndAppendIpsForDualWithIPFamily(allocatedIPs, macStr, podName, nicName, subnet, checkConflict, ipFamily)
	if err != nil {
		klog.Errorf("failed to append allocate ip %v mac %v for %s", allocatedIPs, mac, podName)
		if len(allocatedIPs) != 0 {
			subnet.ReleaseAddressWithNicName(podName, nicName)
		}
		return "", "", "", err
	}
	allocatedIPs = completedIPs
...
func isValidIPFamily(ipFamily string) bool {
	return ipFamily == "" || ipFamily == util.IPFamilyIPv4 || ipFamily == util.IPFamilyIPv6
}

func checkAndAppendIpsForDual(ips []IP, mac, podName, nicName string, subnet *Subnet, checkConflict bool) ([]IP, error) {
	return checkAndAppendIpsForDualWithIPFamily(ips, mac, podName, nicName, subnet, checkConflict, "")
}

func checkAndAppendIpsForDualWithIPFamily(ips []IP, mac, podName, nicName string, subnet *Subnet, checkConflict bool, ipFamily string) ([]IP, error) {
